...
package com.example.warehouseandroid.documentitemedit.view
...
@Composable
fun DocumentItemEditScreen(documentItemJson: String, onGoBackRequested: () -> Unit) {
    val viewModel = koinViewModel<DocumentItemEditViewModel> { parametersOf(documentItemJson) }
    ...
    Scaffold(topBar = {
        TopAppBar(
            title = { Text(stringResource(id = R.string.edit_document_item)) },
            colors = TopAppBarDefaults.largeTopAppBarColors(
                containerColor = MaterialTheme.colorScheme.primaryContainer,
                titleContentColor = MaterialTheme.colorScheme.primary
            ),
            actions = {
                IconButton(onClick = { (viewModel.putDocumentItem()) }) {
                    Icon(
        ...
        if (isLoading) {
            ...
            Column(
                ...
                OutlinedTextField(
                    ...
                    keyboardActions = KeyboardActions(
                        //todo
                        onAny = { viewModel.putDocumentItem() }),
                    modifier = Modifier
                        .fillMaxWidth()
                        .padding(16.dp)
